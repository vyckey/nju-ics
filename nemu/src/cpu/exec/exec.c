...
opcode_entry opcode_table [512] = {
  ...
  /* 0x68 */	IDEX(I, push), IDEX(I_E2G, imul3), IDEXW(SI, push, 1), IDEX(SI_E2G, imul3),
  ...
  /* 0x90 */	EX(nop), IDEX(xchg, xchg), IDEX(xchg, xchg), IDEX(xchg, xchg),
  ...
  /* 0xc8 */	EMPTY, EX(leave), EMPTY, EX(ret),
...
static inline void update_eip(void) {
  if (decoding.is_jmp && decoding.jmp_eip==0x101632) printf("->%x\n", cpu.eip);
  if (decoding.is_jmp && decoding.jmp_eip<0x100000) printf("<-%x\n", cpu.eip);
